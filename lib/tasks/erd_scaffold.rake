  ...
  task generate: :environment do
    ...
    setting_modules.each do |m|
      ...
      search_str =  tables["#{m}"].map do |field|
        next if ["password_digest", "content"].include?(field[:name])
        if field[:fk] == true
    ...
    setting_modules.each do |m|
      m = m.singularize
      g =  "rails g actor #{m}::list --force"
      system(g)
      g =  "rails g actor #{m}::create --force"
      system(g)
      g =  "rails g actor #{m}::update --force"
      system(g)
      g =  "rails g actor #{m}::find --force "
      system(g)
    end

    setting_modules.each do |m| # users
      module_name = m.singularize # user
      class_name = m.singularize.camelize # User
      list_file = "app/actors/#{module_name}/list.rb"
      find_file = "app/actors/#{module_name}/find.rb"
      create_file = "app/actors/#{module_name}/create.rb"
      update_file = "app/actors/#{module_name}/update.rb"

      params_str = tables["#{m}"].map do |field|
        ":#{field[:name]}"
      end.join(", ")
      ...
      class #{class_name}::List < Actor
        output :all_#{m}
        output :enabled_#{m}

        def call
          self.all_#{m} = #{class_name}.where(state: %w[default enable disable])
                                          .order(state: :desc)

          self.enabled_#{m} =#{class_name}.where(state: %w[default enable])
                                              .order(state: :desc)
        end
      end
      RUBY
      ...
      puts "已重寫 actor 檔案: #{list_file}"

      create_content = <<~RUBY
      # frozen_string_literal: true

      class #{class_name}::Create < Actor
        input :params

        output :#{module_name}

        def call
          self.#{module_name} = #{class_name}.new(#{module_name}_params)
          return if #{module_name}.save

          fail!(error: #{module_name}.formatted_errors)
        end

        private

        def #{module_name}_params
          params.permit(#{params_str})
        end
      end
      RUBY

      File.write(create_file, create_content)
      puts "已重寫 actor 檔案: #{create_file}"

      update_content = <<~RUBY
      # frozen_string_literal: true

      class #{class_name}::Update < Actor
        input :#{module_name}_id
        input :params

        output :#{module_name}

        play #{class_name}::Find,
            :update_#{module_name}

        private

        def update_#{module_name}
          return if #{module_name}.update(#{module_name}_params)
      ...
      File.write(update_file, update_content)
      puts "已重寫 actor 檔案: #{update_file}"

      find_content = <<~RUBY
      # frozen_string_literal: true

      class #{class_name}::Find < Actor
        input :#{module_name}_id

        output :#{module_name}

        def call
          #{module_name} = #{class_name}.find_by(id: #{module_name}_id)
          if #{module_name}.present?
            self.#{module_name} = #{module_name}
          else
            fail!(error: I18n.t("actor.#{module_name}.not_found"))
          end
        end
      end
      RUBY

      File.write(find_file, find_content)
      puts "已重寫 actor 檔案: #{find_file}"
    end

    actor_local_file = "config/locales/zh-TW/actor.yml"
    controller_local_file = "config/locales/zh-TW/controller.yml"

    FileUtils.mkdir_p(File.dirname(actor_local_file))
    FileUtils.mkdir_p(File.dirname(controller_local_file))

    actor_local_str = setting_modules.map do |m|
    ...
    s = <<~RUBY
    #{m.singularize}:
      not_found: \"#{m.singularize}不存在\"
    RUBY
      end
      s
    end.join("\n")

    controller_local_str = setting_modules.map do |m|
      s = <<~RUBY
      #{m}:
        create:
          success: "#{m}新增成功"
    ...
    controller_local_content = <<~RUBY
      zh-TW:
        application:
          need_login: "重新登入"
          no_permission: "無權限使用此功能"
          wrong_login: "錯誤的登入窗口"
        session:
          create:
            success: "登入成功"
          destroy:
            success: "登出成功"
        setting:
          #{controller_local_str}
    RUBY

    File.write(actor_local_file, actor_local_content)
    puts "已重寫 locale 檔案: #{actor_local_file}"

    File.write(controller_local_file, controller_local_content)
    puts "已重寫 locale 檔案: #{controller_local_file}"
  end
end
